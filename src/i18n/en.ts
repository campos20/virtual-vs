...
export const en = {
  ...
  folder: {
    ...
    createFailed:
      "Couldn't create the folder. Check that this device has free space.",
  ...
  progress: {
    ...
    decodingOf: (current: number, total: number) =>
      `Decoding stems (${current} of ${total})…`,
  ...
  markers: {
    heading: "Markers",
    emptyText:
      "No markers yet. Tap a preset or type a name, then add one at the current position.",
  ...
  lyrics: {
    ...
    editHint:
      "Saving clears any existing tap-to-sync timing, since the lines may no longer match up.",
    tapHint: "Tap a line to fine-tune sync",
    allCaps: "ALL CAPS",
    syncCount: (count: number) =>
      `${count} line${count === 1 ? "" : "s"} synced`,
