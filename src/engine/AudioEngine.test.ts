...
function decoded(engine: AudioEngine, m: ProjectManifest): DecodedProject {
  ...
  for (const t of m.tracks) {
    trackBuffers[t.id] = engine.context.createBuffer(
      1,
...
function recordScheduling(engine: AudioEngine): {
  ...
  jest.spyOn(engine.context, "createBufferSource").mockImplementation(() => {
    ...
    jest
      .spyOn(node, "start")
      .mockImplementation((when?: number) => starts.push(when));
    jest
      .spyOn(node, "stop")
      .mockImplementation((when?: number) => stops.push(when));
    return node;
  });
  return { starts, stops };
}
