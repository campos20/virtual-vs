...
describe("folder writes", () => {
  it("writes the file before updating the store", () => {
    ...
    expect(
      setlistsSelectors.selectById(store.getState().setlists, "set")?.songs,
    ).toEqual(["song"]);
  });
...
describe("deleteFolder", () => {
  ...
  it("keeps the folder when the delete fails", () => {
    ...
    expect(setlistsSelectors.selectIds(store.getState().setlists)).toEqual([
      "set",
    ]);
  });
});
...
describe("removeSongFromAllFolders", () => {
  it("drops a deleted project from every folder that listed it, and only those", () => {
    ...
    expect(setlistsSelectors.selectById(state, "sunday")?.songs).toEqual([
      "kept",
    ]);
    expect(setlistsSelectors.selectById(state, "wedding")?.songs).toEqual([]);
    expect(setlistsSelectors.selectById(state, "rehearsal")?.songs).toEqual([
      "kept",
    ]);
