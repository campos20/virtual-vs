...
function storeWithProject(overrides: Partial<LibraryProjectEntry> = {}) {
  ...
  store.dispatch(
    tracksInitializedForProject({ projectId: entry.id, tracks: entry.tracks }),
  );
  return store;
}
...
describe("persistProjectMixer", () => {
  it("writes committed volume, bus, mute and solo into the manifest", () => {
    ...
    store.dispatch(
      trackBusSet({ projectId: "song", trackId: "bass", bus: "cue" }),
    );
  ...
  it("mirrors the persisted mix back onto the library entry", () => {
    ...
    store.dispatch(
      trackVolumeCommitted({ projectId: "song", trackId: "bass", volume: 0.3 }),
    );
    store.dispatch(persistProjectMixer("song"));

    expect(store.getState().projects.entities.song?.tracks[0]).toMatchObject({
      gain: 0.3,
    });
  });
...
describe("persistProjectLyrics", () => {
  it("writes the lyrics text into the manifest and the entry, clearing sync points", () => {
    ...
    expect(store.getState().projects.entities.song?.lyrics).toBe(
      "La la la\nVerse two",
    );
