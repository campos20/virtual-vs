...
describe("setlistsSlice", () => {
  it("adds a setlist", () => {
    ...
    expect(setlistsSelectors.selectById(state, "set1")?.songs).toEqual([
      "proj1",
      "proj2",
    ]);
  });
  ...
  it("upserts an existing setlist in place", () => {
    ...
    const upserted = reducer(
      added,
      setlistUpserted(buildEntity({ advance: "auto" })),
    );
    expect(setlistsSelectors.selectIds(upserted)).toEqual(["set1"]);
    expect(setlistsSelectors.selectById(upserted, "set1")?.advance).toBe(
      "auto",
    );
  });
  ...
  it("applies a partial update", () => {
    ...
    const updated = reducer(
      added,
      setlistUpdated({ id: "set1", changes: { padBetween: true } }),
    );
