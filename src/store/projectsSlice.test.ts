...
describe("projectsSlice", () => {
  it("adds a project", () => {
    ...
    expect(projectsSelectors.selectById(state, "proj1")).toMatchObject({
      title: "My Song",
    });
  });
  ...
  it("upserts an existing project in place", () => {
    ...
    const upserted = reducer(
      added,
      projectUpserted(buildEntry({ title: "Renamed Set" })),
    );
    expect(projectsSelectors.selectIds(upserted)).toEqual(["proj1"]);
    expect(projectsSelectors.selectById(upserted, "proj1")?.title).toBe(
      "Renamed Set",
    );
  });
  ...
  it("applies a partial update", () => {
    ...
    const updated = reducer(
      added,
      projectUpdated({ id: "proj1", changes: { bpm: 140 } }),
    );
    expect(projectsSelectors.selectById(updated, "proj1")).toMatchObject({
      bpm: 140,
      title: "My Song",
    });
  });
  ...
  it("tracks filesystem origin projects with their source directory", () => {
    const entry = buildEntry({
      id: "proj2",
      origin: "filesystem",
      sourceDir: "file:///projects/proj2",
    });
    const state = reducer(initialState, projectAdded(entry));
    expect(projectsSelectors.selectById(state, "proj2")?.sourceDir).toBe(
      "file:///projects/proj2",
    );
  });
  ...
  it("replaces the library on hydration and records that the disk was read", () => {
    ...
    const rescanned = reducer(
      hydrated,
      projectsHydrated([buildEntry({ id: "b" })]),
    );
