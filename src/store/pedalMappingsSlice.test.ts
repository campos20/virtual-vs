...
describe("pedalMappingsSlice", () => {
  it("adds a mapping", () => {
    ...
    expect(pedalMappingsSelectors.selectById(state, "pedal1")).toMatchObject({
      action: "playPause",
    });
  });
  ...
  it("upserts an existing mapping in place", () => {
    ...
    const upserted = reducer(
      added,
      pedalMappingUpserted(buildMapping({ action: "nextSection" })),
    );
    expect(pedalMappingsSelectors.selectIds(upserted)).toEqual(["pedal1"]);
    expect(pedalMappingsSelectors.selectById(upserted, "pedal1")?.action).toBe(
      "nextSection",
    );
  });
