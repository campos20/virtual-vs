...
describe("tracksSlice", () => {
  ...
  describe("tracksInitializedForProject", () => {
      ...
      const reopened = reducer(
        ...
        tracksInitializedForProject({
          ...
          tracks: manifestTracks.map((t) =>
            t.id === "bass"
              ? { ...t, gain: 0.55, muted: true, soloed: true }
              : t,
          ),
        }),
      );
    ...
    it("treats absent mute/solo in a manifest as off", () => {
      ...
      expect(tracksSelectors.selectById(state, "proj1:bass")?.muted).toBe(
        false,
      );
      expect(tracksSelectors.selectById(state, "proj1:bass")?.soloed).toBe(
        false,
      );
    });
  });
  ...
  it("toggles mute independently per track", () => {
    ...
    const muted = reducer(
      seeded,
      trackMuteToggled({ projectId: "proj1", trackId: "bass" }),
    );
    ...
    const unmuted = reducer(
      muted,
      trackMuteToggled({ projectId: "proj1", trackId: "bass" }),
    );
    expect(tracksSelectors.selectById(unmuted, "proj1:bass")?.muted).toBe(
      false,
    );
  });
  ...
  it("toggles solo", () => {
    ...
    const soloed = reducer(
      seeded,
      trackSoloToggled({ projectId: "proj1", trackId: "bass" }),
    );
  ...
  it("sets the routed bus", () => {
    ...
    const next = reducer(
      seeded,
      trackBusSet({ projectId: "proj1", trackId: "bass", bus: "both" }),
    );
  ...
  it("is a no-op when toggling/committing an unknown track", () => {
    expect(
      reducer(
        initialState,
        trackMuteToggled({ projectId: "proj1", trackId: "nope" }),
      ),
