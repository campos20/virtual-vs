...
describe("importBundleIntoLibrary", () => {
  it("adds the projects the bundle brought", async () => {
    ...
    expect(projectsSelectors.selectIds(store.getState().projects)).toEqual([
      "a",
      "b",
    ]);
  });
  ...
  it("creates a folder the bundle brought and puts it at the top of the library", async () => {
    ...
    resolvesTo({
      projects: [project("a")],
      folders: [folder("sunday", ["a"], "Sunday Set")],
    });

    await store.dispatch(importBundleIntoLibrary(bundleFile));

    expect(
      setlistsSelectors.selectById(store.getState().setlists, "sunday")?.name,
    ).toBe("Sunday Set");
    expect(writeSetlist).toHaveBeenCalledWith(
      folder("sunday", ["a"], "Sunday Set"),
    );
  ...
  it("merges into a folder already here instead of replacing it", async () => {
    ...
    expect(
      setlistsSelectors.selectById(store.getState().setlists, "sunday")?.songs,
    ).toEqual(["mine", "theirs"]);
  });
