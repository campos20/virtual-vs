 ...
 */
export function MonitorSplitSwitch({
  mode,
  onChange,
}: MonitorSplitSwitchProps) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        <Text style={[styles.label, mode === "split" && styles.labelActive]}>
          {t.monitorSplit.split}
        </Text>
        <Switch
          value={mode === "monitor"}
          onValueChange={(isMonitor) =>
            onChange(isMonitor ? "monitor" : "split")
          }
          trackColor={{ false: colors.border, true: colors.accent }}
        />
        <Text style={[styles.label, mode === "monitor" && styles.labelActive]}>
          {t.monitorSplit.sum}
        </Text>
      </View>
    </View>
