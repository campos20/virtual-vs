...
describe("valueFromDrag", () => {
  ...
  it("moves by exactly the distance the finger moved", () => {
    expect(valueFromDrag(0.5, -HEIGHT / 2, HEIGHT, MAX)).toBeCloseTo(
      0.5 + MAX / 2,
      10,
    );
    expect(valueFromDrag(0.8, HEIGHT / 4, HEIGHT, MAX)).toBeCloseTo(
      0.8 - MAX / 4,
      10,
    );
  });
...
describe("classifyGestureEnd", () => {
  ...
  it("calls a second quick tap a double-tap", () => {
    expect(classifyGestureEnd(0, 1000, 1000 - DOUBLE_TAP_MS + 10)).toBe(
      "double-tap",
    );
  });
