...
function manifest(id: string, title = id): ProjectManifest {
  return {
    ...
    key: "",
    tracks: [
      { id: "bass", name: "Bass", file: "bass.wav", gain: 1, bus: "main" },
    ],
    sections: [],
  };
}
...
function roundTrip(source: BundleHeader): BundleHeader {
  ...
  return decodeBundleHeader(
    bytes.subarray(BUNDLE_PREAMBLE_BYTES, BUNDLE_PREAMBLE_BYTES + headerLength),
  );
}
...
describe("bundle preamble", () => {
  it("starts with the magic and the format version", () => {
    ...
    expect(decodeBundlePreamble(bytes)).toMatchObject({
      version: BUNDLE_VERSION,
    });
  });
...
describe("bundle header", () => {
  it("round-trips projects and the folders that group them", () => {
    const source = header({
      ...
      projects: [
        {
          manifest: manifest("song"),
          files: [{ name: "bass.wav", size: 100 }],
        },
        {
          manifest: manifest("other"),
          files: [{ name: "keys.wav", size: 200 }],
        },
      ],
    });
    ...
    expect(decoded.projects.map((p) => p.manifest.id)).toEqual([
      "song",
      "other",
    ]);
  });
  ...
  it("preserves accented and non-Latin titles exactly", () => {
    ...
    expect(roundTrip(source).projects.map((p) => p.manifest.title)).toEqual(
      titles,
    );
  });
  ...
  it("preserves characters outside the BMP, which travel as surrogate pairs", () => {
    const source = header({
      projects: [{ manifest: manifest("song", "Set 🎸"), files: [] }],
    });
  ...
  it("writes the header JSON as pure ASCII, so no byte can be mis-decoded", () => {
    const bytes = encodeBundleHeader(
      header({ projects: [{ manifest: manifest("s", "Coração"), files: [] }] }),
    );
