...
export function encodeBundleHeader(header: BundleHeader): Uint8Array {
  ...
  for (let i = 0; i < BUNDLE_MAGIC.length; i++)
    view.setUint8(i, BUNDLE_MAGIC.charCodeAt(i));
  view.setUint32(4, header.version, true);
  view.setUint32(8, json.length, true);
  for (let i = 0; i < json.length; i++)
    view.setUint8(BUNDLE_PREAMBLE_BYTES + i, json.charCodeAt(i));
 ...
 */
export function decodeBundlePreamble(bytes: Uint8Array): {
  version: number;
  headerLength: number;
} {
  if (bytes.length < BUNDLE_PREAMBLE_BYTES) {
    throw new BundleFormatError(
      "Not a Virtual VS bundle: the file is too short.",
    );
  }
...
export function decodeBundleHeader(bytes: Uint8Array): BundleHeader {
  ...
  if (
    header?.format !== "virtual-vs-bundle" ||
...
export function bundleSize(header: BundleHeader): number {
  const payload = header.projects.reduce(
    (total, project) =>
      total + project.files.reduce((sum, file) => sum + file.size, 0),
    0,
  );
