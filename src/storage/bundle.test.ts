...
function wipeLibrary() {
  ...
  for (const dir of [...disk.directories]) {
    if (dir.startsWith("file:///document/projects/"))
      disk.directories.delete(dir);
  }
}
...
describe("planBundle", () => {
  it("drops store-only fields, so nothing device-specific travels", () => {
    ...
    const [bundled] = planBundle({
      projects: [project("song", ["bass.wav"])],
      folders: [],
    }).projects;
  ...
  it("refuses a project whose audio is missing rather than writing a broken bundle", () => {
    expect(() =>
      planBundle({ projects: [project("song", ["gone.wav"])], folders: [] }),
    ).toThrow(/missing its file gone.wav/);
  });

  it("refuses a project with no folder on this device", () => {
    const entry = { ...project("song", []), sourceDir: undefined };

    expect(() => planBundle({ projects: [entry], folders: [] })).toThrow(
      /no folder on this device/,
    );
  });
});
...
describe("round trip", () => {
  it("restores every stem byte for byte, with its manifest and folder", async () => {
    ...
    const contents = {
      projects: [
        project("sunday-a", ["bass.wav", "keys.wav"], "Coração"),
        project("sunday-b", ["vox.wav"]),
      ],
    ...
    expectSameBytes(
      disk.files.get("file:///document/projects/sunday-a/bass.wav"),
      bass,
    );
    expectSameBytes(
      disk.files.get("file:///document/projects/sunday-a/keys.wav"),
      keys,
    );
    expectSameBytes(
      disk.files.get("file:///document/projects/sunday-b/vox.wav"),
      vox,
    );
    ...
    expect(result.folders).toEqual([
      folder("sunday", ["sunday-a", "sunday-b"]),
    ]);
  });
  ...
  it("survives files larger than one chunk", async () => {
    ...
    expectSameBytes(
      disk.files.get("file:///document/projects/song/big.wav"),
      big,
    );
    // The file after the big one proves the reader resumed at the right offset.
    expectSameBytes(
      disk.files.get("file:///document/projects/song/after.wav"),
      after,
    );
  });
  ...
  it("reads the index without unpacking anything", async () => {
    ...
    const bundle = await writeBundle(
      {
        projects: [project("song", ["bass.wav"], "My Song")],
        folders: [folder("set", ["song"])],
      },
      new File(Paths.cache, "out.vvs"),
    );
    wipeLibrary();
    ...
    expect(disk.files.has("file:///document/projects/song/bass.wav")).toBe(
      false,
    );
  });
});
...
describe("importBundle", () => {
  it("leaves an existing project alone and says so, so re-importing a backup is a no-op", async () => {
    ...
    expectSameBytes(
      disk.files.get("file:///document/projects/song/bass.wav"),
      edited,
    );
    expect(
      firstDifference(
  ...
  it("finishes an import that died before its manifest was written", async () => {
    ...
    disk.files.set(
      "file:///document/projects/song/bass.wav",
      new Uint8Array(7),
    );
    ...
    expect(disk.files.has("file:///document/projects/song/manifest.json")).toBe(
      true,
    );
  });
  ...
  it("re-imports over a manifest that is there but unreadable", async () => {
    ...
    wipeLibrary();
    disk.directories.add("file:///document/projects/song");
    new File("file:///document/projects/song", "manifest.json").write(
      "{ truncated",
    );
  ...
  it("writes the manifest only after the audio it describes", async () => {
    ...
    const stemWrite = disk.log.findIndex((line) =>
      line.includes("projects/song/bass.wav"),
    );
    const manifestWrite = disk.log.findIndex((line) =>
      line.includes("projects/song/manifest.json"),
    );
  ...
  it("refuses a file that is not a bundle", async () => {
    ...
    await expect(importBundle(notABundle)).rejects.toThrow(
      /Not a Virtual VS bundle/,
    );
  });
