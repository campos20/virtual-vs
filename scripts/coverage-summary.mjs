...
try {
  ...
  console.error(
    "No coverage/coverage-summary.json - run jest with coverage first.",
  );
  process.exit(1);
}
...
if (ranked.length > 0) {
  lines.push(
    `<details><summary>Least covered files (${ranked.length})</summary>`,
    "",
  );
  ...
  for (const file of ranked) {
    lines.push(
      `| \`${file.path}\` | ${badge(file.pct)} ${file.pct.toFixed(1)}% | ${file.uncovered} |`,
    );
  }
  lines.push("", "</details>");
}
